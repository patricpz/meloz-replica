...
import bannerMobile2 from '../assets/mockup/banner_mobile2.png'
...
function MobileCarousel({ images, interval = 5000 }) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [nextIndex, setNextIndex] = useState(1)
  const [isTransitioning, setIsTransitioning] = useState(false)

  const getMobileImageClass = (image) => {
    if (image === bannerMobile2) {
      return 'absolute left-1/2 top-1/2 h-full w-[100%] max-w-[480px] -translate-x-1/2 -translate-y-1/2 object-contain'
    }
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-gray-100 flex items-center justify-center">
      <motion.img
        src={images[currentIndex]}
        alt="Banner Mobile"
        className={getMobileImageClass(images[currentIndex])}
        initial={{ opacity: 1 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.8 }}
      />
      ...
      <motion.img
        src={images[nextIndex]}
        alt="Banner Mobile Próximo"
        className={getMobileImageClass(images[nextIndex])}
        initial={{ opacity: 0 }}
        animate={{ opacity: isTransitioning ? 1 : 0 }}
        transition={{ duration: 0.6 }}
      />
...
export default function Home() {
  ...
  const mobileImages = [bannerMobile, bannerMobile2]
  ...
  return (
    <section className="relative min-h-[100svh] w-full mx-auto overflow-hidden pt-20 md:pt-28 lg:pt-32">
      ...
      <div className="absolute inset-0 block md:hidden">
        <MobileCarousel images={mobileImages} interval={5000} />
      </div>
